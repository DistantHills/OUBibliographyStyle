...
<!-- need section name for OU material.  Do by using site name parameter -->
	<xsl:template name="BibDisplayInternetPageName">
		<xsl:variable name="cInternetSiteTitle">
			<xsl:value-of select="count(b:InternetSiteTitle)"/>
		</xsl:variable>
		<xsl:if test ="$cInternetSiteTitle!=0">
			<xsl:text>&apos;</xsl:text>
			<xsl:call-template name="right-trim">
				<xsl:with-param name ="s" select="b:InternetSiteTitle"/>
			</xsl:call-template>
			<xsl:text>&apos;</xsl:text>
			<xsl:text>,</xsl:text>
<!--			<xsl:call-template name ="templ_prop_Dot"/> -->
			...
			<xsl:choose>
				...
				<xsl:when test="$SourceType = 'InternetSite'">
					<xsl:element name="p">
						...
						<xsl:call-template name = "BibDisplayAuthor"/>
						<xsl:call-template name = "BibDisplayYear"/>
						<xsl:call-template name = "BibDisplayInternetPageName"/>
						<i>
							<xsl:call-template name ="BibDisplayTitle"/>
						</i>
